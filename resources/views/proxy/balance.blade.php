            ...
            <section class="w3-brad-10px">
                ...
                @if ($balance > 0)
                    <div class="w3-row-padding w3-border-top">
                        <div class="w3-col">
                            <button class="w3-button w3-black w3-medium w3-padding"
                                onclick="document.getElementById('withdraw').style.display='block'">{{ __('general.balance_withdraw') }}</button>
                        </div>
                    </div>
                @endif
            </section>
            ...
            <div class="w3-modal-content" style="max-width: 450px;">
                @if (empty($vendor->payout))
                    <header class="w3-container w3-border-bottom">
                        <span onclick="document.getElementById('withdraw').style.display='none'"
                    ...
                    <div class="w3-container">
                        <form class="w3-panel vi-pament-wrpr w3-p-0 w3-mb-0" href="{{ route('proxy.add_payout') }}"
                            id="payout_form" onsubmit="return productGenerator.addPayout(event);">
                                            ...
                                            <div class="w3-row-padding">
                                                <div class="w3-col">
                                                    <input class="w3-input w3-border w3-animate-input" type="text"
                                                        name="account" placeholder="Paypal email ID" id="paypal_id" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <footer class="w3-container w3-padding">
                        <button class="w3-button w3-black w3-right" type="submit" form="payout_form">Add payout
                            method</button>
                        <button onclick="document.getElementById('withdraw').style.display='none'"
                            class="w3-button w3-white w3-border w3-right w3-margin-right">cancel</button>
                    </footer>
                @else
                    <header class="w3-container w3-border-bottom">
                        <span onclick="document.getElementById('withdraw').style.display='none'"
                    ...
                    <div class="w3-container">
                        <form class="w3-panel vi-pament-wrpr w3-p-0 w3-mb-0" href="{{ route('proxy.balance.store') }}"
                            id="withdraw_form">
                            <input type="hidden" name="vendor_id" value="{{ $vendor->id }}" />
                            <input type="hidden" name="seller_id" value="{{ $seller->id }}" />
                            ...
                            <table class="w3-table-all">
                                <tr>
                                    <th>Transfer To</th>
                                    <td class="w3-text-capitalized"><img
                                            src="{{ asset('images/icons/' . $vendor->payout->type . '-icon.png') }}"
                                            width="20" /> {{ $vendor->payout->type }} Account</td>
                                </tr>
                                ...
                                <tr>
                                    <th>Amount</th>
                                    <td>
                                        <input type="number" step="0.01" id="withdraw_amt" name="amount" required
                                            class="w3-mb-0" max="{{ $balance }}" value="{{ $balance }}">
                                    </td>
                                </tr>
                            </table>
                        </form>
                    ...
                    <footer class="w3-container w3-padding">
                        <button class="w3-button w3-black w3-right" onclick="productGenerator.withdraw();">Confirm &
                            Withdraw</button>
                        <button onclick="document.getElementById('withdraw').style.display='none'"
                            class="w3-button w3-white w3-border w3-right w3-margin-right">cancel</button>
                        <button class="w3-btn w3-white w3-border w3-border-red w3-text-red"
                            onclick="productGenerator.payoutReset(this);" data-href="{{ route('proxy.payout.reset') }}"
                            data-vendor="{{ $vendor->id }}">Reset</button>
                    </footer>
                @endif
            </div>
        </div>
            ...
            <div class="w3-col s12">
                <table class="w3-table w3-bordered w3-striped w3-hover w3-card">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (!empty($transaction->order_id))
                                        <a
                                            href="/a/seller/orders/{{ $transaction->order_id }}">#{{ $transaction->order->order_number }}</a>
                                    @else
                                        -
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center">No balance history</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
