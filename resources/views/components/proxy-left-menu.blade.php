<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:240px;" id="mySidebar"><br>
    ...
    <div class="w3-bar-block">
        <a class="w3-bar-item w3-button w3-padding  @if($routeName=='proxy.dashboard') w3-text-teal @endif" href="/a/seller">
            <i class="fa fa-home w3-margin-right"></i> {{ ucwords(trans('general.dashboard')) }}
        </a>
        <a class="w3-bar-item w3-button w3-padding @if($routeName=='proxy.orders.index') w3-text-teal @endif" href="/a/seller/orders">
            <i class="fa fa-list w3-margin-right"></i> {{ ucwords(trans('general.orders')) }}
        </a>
        <a class="w3-bar-item w3-button w3-padding @if(in_array($routeName,['proxy.products.index','proxy.products.edit','proxy.products.show'])) w3-text-teal @endif" href="/a/seller/products">
            <i class="fa fa-th-large w3-margin-right"></i> {{ ucwords(trans('general.products')) }}
        </a>
        @if(isset($vendor) AND !empty($vendor))
            <a class="w3-bar-item w3-button w3-padding @if($routeName=='profile') w3-text-teal @endif" href="/a/seller/profile">
                <i class="fa fa-user w3-margin-right"></i> {{ ucwords(trans('general.manage_profile')) }}
            </a>
        @endif
        <a class="w3-bar-usd w3-button w3-padding @if($routeName=='proxy.balance.index') w3-text-teal @endif" href="/a/seller/balance">
            <i class="fa fa-usd w3-margin-right"></i> {{ ucwords(trans('general.balance')) }}
        </a>
        <a class="w3-bar-item w3-button w3-padding @if($routeName=='proxy.subscriptions') w3-text-teal @endif" href="/a/seller/plan/subscriptions">
